...
public class MmsSmsProvider extends ContentProvider {
    ...
    /* Begin add for RCS */
    private static final int URI_FAVOURITE_MESSAGES                = 24;
    private static final int URI_UPDATE_THREAD_TOP                 = 25;
    // Add for 1-n message status.
    private static final int URI_ONE_TO_MANY_MESSAGE_STATUS        = 26;
    // Add for RCS device api delete message
    private static final int URI_DELETE_CONVERSATION_MESSAGE       = 27;
    private static final int URI_DEVICE_API_CONVERSATIONS          = 28;
    private static final int URI_DEVICE_API_MESSAGES               = 29;
    private static final int URI_DEVICE_API_PUBLIC_ACCOUNT_MESSAGE = 30;
    private static final int URI_DEVICE_API_FILE_TRANSFER_MESSAGE  = 31;
    /* End add for RCS */
    ...
    private static String[] sSmsOnlyColumns = DEFAULT_SMS_ONLY_COLUMNS;
    /* End add for RCS */
    ...
    // message tables.
    private static  String[] sUnionColumns =
            new String[MMS_SMS_COLUMNS.length
                       + MMS_ONLY_COLUMNS.length
                       + sSmsOnlyColumns.length];
    ...
    private static String[] handleNullMessageProjection(
            String[] projection) {
        return projection == null ? sUnionColumns : projection;
    }
    ...
    private static void initializeColumnSets() {
        ...
        int smsOnlyColumnCount = sSmsOnlyColumns.length;
        ...
        for (int i = 0; i < smsOnlyColumnCount; i++) {
            SMS_COLUMNS.add(sSmsOnlyColumns[i]);
            unionColumns.add(sSmsOnlyColumns[i]);
        }

        int i = 0;
        for (String columnName : unionColumns) {
            sUnionColumns[i++] = columnName;
        }
    }
    ...
    private void setSmsRcsColumns() {
        ...
        sSmsOnlyColumns = RcsMessageProviderConstants.RCS_SMS_ONLY_COLUMNS;
        sUnionColumns = new String[MMS_SMS_COLUMNS.length
                           + MMS_ONLY_COLUMNS.length
                           + sSmsOnlyColumns.length];
        initializeColumnSets();
    }
